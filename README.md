...
- `avaclo-sessions.json`: ログインセッション情報（暗号化済み）
- `avaclo-settings.json`: アバターの設定・マッピング情報
...
### セキュリティ (Security)
- **Encryption**: ログインセッション情報 (`avaclo-sessions.json`) は、Windows DPAPI (Data Protection API) 等を使用して暗号化され、安全に保存されます。
